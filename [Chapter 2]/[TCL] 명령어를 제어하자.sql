...
BEGIN TRAN /* 2. TCL 시작 (TCL 명령어를 시작하기 위한 선언 명령어) */

DELETE  FROM  [회원테이블] /* 4. [회원테이블] 모든 행 데이터 삭제 */

COMMIT /* 모든 행 데이터 삭제 실행 (DML 명령어를 최종 실행하는 명령어) */
...
ROLLBACK /* 6. [회원테이블] 모든 행 데이터 삭제 취소(DML 명령어를 취소하는 명령어) */
...
/* 6. S3 지정 취소 */
ROLLBACK TRAN S3;          -- 'A10003' 회원 데이터 삭제 취소
SELECT * FROM  [회원테이블] -- 확인

/* 7. S2 지정 취소 */
ROLLBACK TRAN S2;          -- 'A10001' 나이 34으로 수정 취소
SELECT * FROM  [회원테이블] -- 확인

/* 8. S1 지정 취소 */
ROLLBACK TRAN S1;          -- 'A10001' 회원 데이터 삽입 취소
SELECT * FROM  [회원테이블] -- 확인